...
## 10. Macro Events Backfill Status

### 10.1 Current `data/macro_state.db` → `macro_events` Table

| Metric | Value |
|--------|-------|
| Total rows | 195 |
| Date range | 2020-01-29 to 2026-03-20 |

**Event type breakdown:**

| Event Type | Count | Notes |
|------------|-------|-------|
| FOMC_SCHEDULED | 47 | 2020-2025 (all 8/yr covered) |
| FOMC_EMERGENCY | 2 | 2020-03-03 and 2020-03-15 (COVID) |
| FOMC | 1 | 2026-03-19 (current) |
| FOMC_POST | 1 | 2026-03-20 (current) |
| CPI | 72 | 2020-2026 (good coverage) |
| CPI_POST | 1 | 2026-03-13 (current) |
| NFP | 71 | 2020-2025 (good coverage) |

**Historical coverage pre-2025:** FOMC=39, CPI=59, NFP=59 — adequate for training data enrichment.

### 10.2 Backfill Gap: POST-event Buffers

The table has historical FOMC/CPI/NFP event dates but is **missing historical POST-event buffer rows** (FOMC_POST, CPI_POST, NFP_POST) for 2020-2025. Only the current week's POST events are present (inserted by `run_daily_event_check()`).

**Impact on ML:** When enriching training data with `event_scaling` features, we need to know if a trade entry date fell on a post-event buffer day. Without historical POST rows, we'd need to compute them on-the-fly from the event dates (add 1 calendar day to each FOMC/CPI/NFP date).

**Recommendation:** Not a blocker — `collect_training_data.py` can derive post-event proximity from the raw event dates. No DB backfill needed.

---

## 11. yfinance Dependency Audit

### 11.1 Summary

**17 source files** import or call yfinance across the codebase, plus **3 test files** that mock it.

### 11.2 By Category

**ML pipeline (3 files — CC-3 scope, must replace with IronVault):**

| File | Lines | Usage |
|------|-------|-------|
| `ml/feature_engine.py` | 19, 56, 321 | `yf.download()` for OHLCV, `yf.Ticker()` for earnings |
| `ml/iv_analyzer.py` | 18, 313 | `yf.download()` for historical IV proxy |
| `ml/collect_training_data.py` | 603, 606, 612 | `yf.download("SPY"/"^VIX")` full history |

**Engine/backtest (2 files — shared scope):**

| File | Lines | Usage |
|------|-------|-------|
| `engine/portfolio_backtester.py` | 17, 182, 219 | Core OHLCV + VIX downloads |
| `backtest/backtester.py` | 145, 1026 | Legacy backtester (deprecated) |

**Shared (2 files — shared scope):**

| File | Lines | Usage |
|------|-------|-------|
| `shared/data_cache.py` | 8, 58, 101 | TTL cache wrapping `yf.Ticker()` |
| `shared/earnings_calendar.py` | 48, 126, 228 | Earnings date lookups |

**Alerts/scanners (5 files — live trading, keep external API):**

| File | Lines | Usage |
|------|-------|-------|
| `alerts/gamma_scanner.py` | 145-146 | `yf.Ticker().history()` |
| `alerts/iron_condor_scanner.py` | 115-116 | `yf.Ticker().history()` |
| `alerts/momentum_scanner.py` | 126-127 | `yf.Ticker().history()` |
| `alerts/zero_dte_scanner.py` | 142-143 | `yf.Ticker().history()` |
| `alerts/earnings_scanner.py` | 175-176 | `yf.Ticker().history()` |

**Scripts (4 files — diagnostic/validation):**

| File | Lines | Usage |
|------|-------|-------|
| `scripts/exp600_trade_flow_debug.py` | 71-72 | SPY download |
| `scripts/paper_trading_deviation.py` | 250, 256, 263 | SPY + VIX download |
| `scripts/live_readiness_check.py` | 610, 615, 622 | SPY + VIX download |
| `scripts/validate_signal_alignment.py` | 143, 158, 177 | SPY + VIX download |

**Tests (3 files — mock patches):**
`tests/test_feature_engine.py`, `tests/test_options_analyzer.py`, `tests/test_data_cache.py`

### 11.3 Replacement Strategy

- **ML files (3):** Replace with IronVault in Phase 3. Priority.
- **Engine/shared (4):** Replace with IronVault for backtesting paths. VIX needs external source.
- **Alerts (5):** Keep live API (Polygon/Alpaca). IronVault is offline-only.
- **Scripts (4):** Low priority. Replace when convenient.
- **Tests (3):** Update mocks to match new data source.

---

*Updated by CC-3 — 2026-03-20*
