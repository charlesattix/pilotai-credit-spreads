...
def validate_config(config: AppConfig) -> None:
    ...
    Populates sensible defaults for optional sections/fields so configs
    generated by the optimization pipeline don't crash at startup.
    ...
    config.setdefault('data', {'provider': 'polygon', 'cache_dir': 'data/options_cache'})
    config.setdefault('backtest', {'starting_capital': 100000})

    required_sections = ['tickers', 'strategy', 'risk']
    ...
    # Delta bounds — default if not specified (not all strategies use delta selection)
    strategy.setdefault('min_delta', 0.08)
    strategy.setdefault('max_delta', 0.18)
    ...
    risk = config['risk']
    risk.setdefault('min_credit_pct', 5)
    try:
