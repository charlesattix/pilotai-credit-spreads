#!/usr/bin/env python3
"""
Run full-year backtests for 2020 and 2021 using the production winning config.

Production config: OTM=3%, Width=$5, MinCredit=10%, SL=2.5x, 2% risk, max 5 contracts

Problem: Polygon's /v3/reference/options/contracts returns empty results for
expired options from 2020/2021 on the free tier. However, the OHLCV endpoints
(/v2/aggs/ticker/{OCC}/range/...) DO return real data for those contracts.

Solution:
  1. Patch _DEFAULT_LOOKBACK_YEARS to 7 so _fetch_and_cache covers 2019-2026.
  2. Pre-seed option_contracts SQLite table with computed strikes so the backtester
     skips the empty reference API and goes straight to OHLCV lookups.
"""
...
def main():
    print("=" * 70)
    print("HISTORICAL BACKTEST — 2020 & 2021 FULL YEARS")
    ...
    periods = [
        ("2020 Full Year", datetime(2020, 1, 1), datetime(2020, 12, 31)),
        ("2021 Full Year", datetime(2021, 1, 1), datetime(2021, 12, 31)),
    ]

    # Pre-seed option_contracts for both years in one pass
    combined_start = datetime(2020, 1, 1)
    combined_end = datetime(2021, 12, 31)
    ...
    # ── Combined file ──────────────────────────────────────────────────────────
    combined_path = out_dir / "backtest_results_2020_2021.json"
